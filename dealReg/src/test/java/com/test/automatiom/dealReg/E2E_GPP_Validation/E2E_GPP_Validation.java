...
import com.test.automatiom.dealReg.testBase.TestBase1;
...
public class E2E_GPP_Validation extends TestBase {
	...
	public void GPP_Validate() throws Exception {
		try {
			...
			driverwait(10);
			clickWhenReady(
					By.xpath("//label[text()='GPP Status:']/following::td[1]"),
					30);
			...
			System.out
					.println("Clicked on Pending option in GPP Status DropDown");
			Reporter.log("Clicked on Pending option in GPP Status DropDown");
			System.out.println("waiting for 15 min.......");
			driverwait(900);
			sendkeysWhenReady(
					By.xpath(".//*[@id='com_ibm_ciolab_dr_support_GppFilter_0_solutionIdFilter']"),
					s, 30);
			driverwait(15);
			driver.findElement(
			...
			java.util.List<WebElement> Table_row = driver
					.findElements(By
	...
	/*
	 * @AfterClass public void tearDown() { closeBrowser();
